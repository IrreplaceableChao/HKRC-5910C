...
public class Amplify_horizontal extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_amplify_h);
        findview();
        setTopBottom((float) zuo, getZY_Max() + 5);
        setonClick();
        init();
    }
    ...
    private LineData getLineData() {
        ...
        for (int i = 0; i < length; i++) {
            x[i] = (float) ((formula.y_()[i] * Math.cos((270 + $E$2 + 0) * $F$1) - formula.x_()[i] * Math.sin((270 + $E$2 + 0) * $F$1)));//2017-7-12 16:00:50 为单旭升单独更改  在X轴*100  所以在这个位置需要除以100
//			y[i] = (int)(formula.y_()[i] * Math.sin((270+ $E$2 + 0) * $F$1) + formula.x_()[i] * Math.cos((270+$E$2 + 0) * $F$1)*100);
            y[i] = (float) (zy[i]);
            if (x[i] < min) min = x[i];
        }
