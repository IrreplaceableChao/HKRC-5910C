 ...
 * 
 */
public class TextWatcherMaxFloat implements TextWatcher {

	private Context mContext;
	private EditText mEditText = null;
	private int mnMaxbytes = 0; // 最大字节数
	private float mnMaxvalue = 0; // 最大值(为-1时不做判断)
	 ...
	 *            --限制最大数值(-1为忽略此限制)
	 */
	public TextWatcherMaxFloat(Context context, EditText editText,
			int maxbytes, float maxvalue) {
		mEditText = editText;
		mContext = context;
		mnMaxbytes = maxbytes;
		mnMaxvalue = maxvalue;
	...
	public void setMaxParams(int maxbytes, float maxvalue) {
		mnMaxbytes = maxbytes;
		mnMaxvalue = maxvalue;
	}
	...
	public void onTextChanged(CharSequence arg0, int arg1, int arg2, int arg3) {
		...
		if ((mnMaxvalue != -1) && (byteLen != 0)) {
			...
			Log.e("aaaaaaaaaaaaaaa", ""+editable.toString());
			if(!(editable.toString().trim()).equals("")){
				if(!(editable.toString().trim()).equals(".")){
				Log.e("if_aaaaaaaaaaaaaaa", ""+editable.toString());
